...
class TestChunkRanges:
    def test_year_splits_into_bounded_chunks(self) -> None:
        ...
        assert all((c_end - c_start) <= timedelta(days=HISTORICAL_CHUNK_DAYS) for c_start, c_end in chunks), (
            "a chunk longer than the cap could exceed visibility_timeout and be redelivered mid-flight"
        )

    def test_covers_the_whole_window_without_gaps_or_overlap(self) -> None:
    ...
    def test_short_window_is_a_single_chunk(self) -> None:
    ...
    def test_exact_multiple_does_not_emit_an_empty_trailing_chunk(self) -> None:
    ...
    def test_non_positive_window_stays_a_single_chunk(self, days: int) -> None:
    ...
    def test_chunk_cap_stays_well_inside_visibility_timeout(self) -> None:
