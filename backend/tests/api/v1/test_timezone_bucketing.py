...
class TestActivityBucketingWithZoneOffset:
    ...
    def test_steps_bucket_to_local_date_via_zone_offset(
        ...
        assert len(data["data"]) == 1, (
            f"Expected 1 day bucket (2026-05-03 local) but got: "
            f"{[r['date'] for r in data['data']]}"
        )
        assert data["data"][0]["date"] == RUN_LOCAL_DATE
        assert data["data"][0]["steps"] == 1500

    def test_steps_do_not_appear_on_wrong_utc_date(
        self, client: TestClient, db: Session
    ) -> None:
        ...
        data = response.json()
        assert data["data"] == [], (
            "Samples with +10:00 offset should not appear in the UTC-Saturday bucket"
        )
...
class TestActivityBucketingWithUserTimezone:
    ...
    def test_steps_bucket_to_local_date_via_user_timezone(
        self, client: TestClient, db: Session
    ) -> None:
        ...
        data = response.json()
        assert len(data["data"]) == 1, (
            f"Expected 1 day bucket (2026-05-03 local via user.timezone) but got: "
            f"{[r['date'] for r in data['data']]}"
        )
        assert data["data"][0]["date"] == RUN_LOCAL_DATE
        assert data["data"][0]["steps"] == 1500

    def test_steps_do_not_appear_on_wrong_utc_date_user_timezone(
        self, client: TestClient, db: Session
    ) -> None:
        ...
        data = response.json()
        assert data["data"] == [], (
            "Samples should not appear in UTC-Saturday bucket when user.timezone=Brisbane"
        )

    def test_zone_offset_takes_precedence_over_user_timezone(
        self, client: TestClient, db: Session
    ) -> None:
        ...
        data = response.json()
        assert len(data["data"]) == 1, (
            "zone_offset=+00:00 should anchor the sample to 2026-05-02, not 2026-05-03"
        )
...
class TestActivityBucketingUtcFallback:
    ...
    def test_steps_bucket_to_utc_date_when_no_timezone(
        self, client: TestClient, db: Session
    ) -> None:
...
class TestSleepBucketingWithZoneOffset:
    ...
    def test_wake_date_uses_zone_offset(
        self, client: TestClient, db: Session
    ) -> None:
        user = UserFactory(timezone=None)
        mapping = DataSourceFactory(user=user, source="garmin")
    ...
    def test_sleep_not_visible_on_wrong_utc_date_zone_offset(
        self, client: TestClient, db: Session
    ) -> None:
...
class TestSleepBucketingWithUserTimezone:
    ...
    def test_wake_date_uses_user_timezone_when_zone_offset_null(
        self, client: TestClient, db: Session
    ) -> None:
    ...
    def test_sleep_not_visible_on_wrong_utc_date_user_timezone(
        self, client: TestClient, db: Session
    ) -> None:
        ...
        assert response.status_code == 200
        assert response.json()["data"] == [], (
            "Session should not appear on UTC date when user.timezone=Brisbane"
        )

    def test_zone_offset_takes_precedence_over_user_timezone_sleep(
        self, client: TestClient, db: Session
    ) -> None:
        ...
        data = response.json()
        assert len(data["data"]) == 1, (
            "zone_offset=+00:00 should anchor wake date to 2026-05-03 (UTC)"
        )
...
class TestSleepBucketingUtcFallback:
    ...
    def test_wake_date_falls_back_to_utc_when_no_timezone(
        self, client: TestClient, db: Session
    ) -> None:
...
class TestBucketingCrossUserIsolation:
    ...
    def test_utc_user_unaffected_by_brisbane_neighbour(
        self, client: TestClient, db: Session
    ) -> None:
